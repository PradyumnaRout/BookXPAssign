...
//            let fetchedData: [PortfolioData] = load("Response.json")
    ...
    func deleteItem(_ id: String, _ item: PortfolioData) {
        let task = Task {
            ...
            if UserDefaultsManager.shared.enableNotification {
                if let title = item.name {
                    NotificationManager.shared.notifyItemDeletion(title: "Item Deleted", body: "\(title) \(item.data?.color ?? "") \(item.data?.capacity ?? "") is no loger available in your portfolio")
                }
            }
            fetchListFromDb()
        }
